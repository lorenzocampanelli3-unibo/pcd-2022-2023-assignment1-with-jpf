...
public class AnalyzerUI implements ActionListener {
    ...
    public AnalyzerUI(Controller controller, int defaultProcCoresMultiplier, boolean isProcCoresMultiplierInputAllowed) {
...
//            fileChooser.setFileFilter(new FolderFilter());
            ...
            if (!maxLinesParam.isEmpty() && !numIntervalsParam.isEmpty() && !snapshotSizeParam.isEmpty() && !procCoresMultiplierParam.isEmpty()) { // FIXME: lacks proper input validation
                ...
                if (procCoresMultiplierInt > 0) {
                    ...
                    this.status.setText("");
                    controller.notifyStarted(chosenDirPath, maxLinesInt, numIntervalsInt, snapshotSizeInt, procCoresMultiplierInt);
                } else {
    ...
    public void updateStats(Stats.Snapshot statsSnapshot) {
        SwingUtilities.invokeLater(() -> {
            this.updateTopResultsTable(statsSnapshot.getFilesLinesCountSnap());
            this.updateDistributionTable(statsSnapshot.getDistributionSnap());
        });
    }
...
//        this.status.setText("");
    ...
    private void updateTopResultsTable(List<Pair<String, Long>> topResultsSnap) {
        topResultsTableModel.setRowCount(0);
        topResultsSnap.forEach(f -> this.topResultsTableModel.addRow(new Object[]{f.getX(), f.getY()}));
    }

    private void updateDistributionTable(Map<Range, Long> distributionSnap) {
        distributionTableModel.setRowCount(0);
        distributionSnap.forEach((r, v) -> this.distributionTableModel.addRow(new Object[]{r, v}));
    }
